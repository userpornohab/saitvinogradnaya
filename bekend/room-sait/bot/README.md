...
`TELEGRAM_API_PROXY` нужен только если Telegram API недоступен напрямую. Подойдут
`http://user:pass@host:port`, `socks4://host:port`, `socks5://host:port`.
...
Когда клиент нажимает кнопку Telegram на сайте, бот отправляет менеджеру сообщение
о новой заявке. Чтобы ответить именно этому клиенту:

1. Откройте чат с ботом от аккаунта менеджера.
2. Нажмите правой кнопкой/долгим тапом на сообщение заявки.
3. Выберите «Ответить».
4. Напишите текст ответа.

Бот отправит этот текст клиенту в его личный чат с ботом. То же работает для
обычных сообщений клиента: бот пересылает их менеджеру, а менеджер отвечает
реплаем на это пересланное сообщение.

Связь `сообщение менеджеру -> клиент` хранится в памяти процесса бота. Если бот
перезапустили, он попробует восстановить клиента по `id` из текста сообщения.

          ...
          <div v-for="amenity in allAmenities" :key="amenity.id" class="selection-item">
            <label>
              <input 
                ...
                @change="toggleAmenitySelection(amenity.id, $event.target.checked)"
              >
              <img :src="getIconUrl(amenity.icon)" :alt="amenity.name" class="icon-small">
              {{ amenity.name }}
              <button 
                @click.stop="$emit('delete-amenity', amenity.id)" 
                class="delete-btn"
          ...
          <div class="custom-file-input">
            <label class="file-input-label">
              ...
              <span class="button-content">
                ...
                {{ newAmenity.iconFile ? newAmenity.iconFile.name : 'Выберите иконку' }}
              </span>
            </label>
          </div>
          <button type="button" @click="addNewAmenity">Добавить</button>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  emits: ['update:selected-amenities', 'add-amenity', 'delete-amenity'],
  data() {
    return {
      showForm: false,
      newAmenity: {
  ...
  methods: {
    ...
    toggleAmenitySelection(amenityId, isChecked) {
      const newSelection = [...this.selectedAmenities];
      
      if (isChecked) {
        newSelection.push(amenityId);
      } else {
        const index = newSelection.indexOf(amenityId);
        if (index > -1) {
          newSelection.splice(index, 1);
        }
      }
      
      this.$emit('update:selected-amenities', newSelection);
    },
    ...
    addNewAmenity() {
      ...
      if (!this.newAmenity.iconFile) {
        alert('Выберите иконку');
        return;
      }
      
      this.$emit('add-amenity', {
        name: this.newAmenity.name,
        iconFile: this.newAmenity.iconFile
      });
